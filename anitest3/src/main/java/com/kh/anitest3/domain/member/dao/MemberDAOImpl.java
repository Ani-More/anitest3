...
//														);
	...
	//일반회원 가입쿼리
	public void join(JoinNormalForm joinNF) {
		StringBuffer sql = new StringBuffer();
		String bcategory = "";
		int rows = 0;
	...
	//특수회원 가입쿼리
	public void join(JoinSpecialForm joinSF) {
		StringBuffer sql = new StringBuffer();
		String bcategory = "";
		int rows = 0;
		...
		//(체크박스 사용시) 카테고리 체크
		int bc_count=0;
		if(joinSF.getBhospital() != null) {
			bcategory+=",bhospital";
			bc_count++;
		}
		if(joinSF.getBpharmacy() != null) {
			bcategory+=",bpharmacy";
			bc_count++;
		}
		if(joinSF.getBhotel() != null) {
			bcategory+=",bhotel";
			bc_count++;
		}
		if(joinSF.getBkindergarden() != null) {
			bcategory+=",bkindergarden";
			bc_count++;
		}
		if(joinSF.getBfood() != null) {
			bcategory+=",bfood";
			bc_count++;
		}
		if(joinSF.getBtraining() != null) {
			bcategory+=",btraining";
			bc_count++;
		}
		if(joinSF.getBshop() != null) {
			bcategory+=",bshop";
			bc_count++;
		}
		if(joinSF.getBplayground() != null) {
			bcategory+=",bplayground";
			bc_count++;
		}
		if(joinSF.getBhairshop() != null) {
			bcategory+=",bhairshop";
			bc_count++;
		}
		if(joinSF.getBetc() != null) {
			bcategory+=",betc";
			bc_count++;
		}
		//카테고리 갯수만큼 bcategory values에 'Y' 추가
		...
		if(joinSF.getBhospital() != null) {
			...
			if(joinSF.getNightcare() != null)	h_tags_value+=",'Y'";
			else h_tags_value+=",'N'";
			if(joinSF.getRareani() != null) h_tags_value+=",'Y'";
			else h_tags_value+=",'N'";
			if(joinSF.getVisitcare() != null) h_tags_value+=",'Y'";
			else h_tags_value+=",'N'";
			if(joinSF.getHolidayopen() != null) h_tags_value+=",'Y'";
			else h_tags_value+=",'N'";
			if(joinSF.getDental() != null) h_tags_value+=",'Y'";
			else h_tags_value+=",'N'";
		}
...
//															);
		...
		//자격증정보
		if(!joinSF.getLicenseno().isBlank()) {
			sql.append("into profession(PNUM,PID,LICENSENO) values(profession_pnum_seq.nextval,?,?) ");
		}
		...
		// 자격증, 사업정보 둘다 기입한경우
		if((!joinSF.getLicenseno().isBlank()) && (bc_count != 0)) {
			log.info("1");
			rows = jdbcTemplate.update(
					sql.toString(),
					joinSF.getId(),
